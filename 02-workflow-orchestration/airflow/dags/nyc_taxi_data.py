...
dag = DAG(
    'nyc_taxi_data_pipeline_v2',
    default_args=default_args,
    description='Download NYC Taxi data (Parquet) - answers Q1',
    schedule=None,
    catchup=False,
    tags=['homework', 'taxi', 'q1', 'fixed'],
)
...
def download_taxi_data(**context):
    ...
    # Try parquet first (newer data format)
    ...
    try:
        ...
        # Read parquet to count rows
        df = pd.read_parquet(filepath)
        row_count = len(df)
        
        print("=" * 70)
        print(f"✅ HOMEWORK Q1 ANSWER:")
